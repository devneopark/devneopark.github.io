...
def _read_front_matter(path: Path) -> tuple[dict[str, Any], str]:
    ...
    try:
        ...
        raise PostParseError(f"{path}: YAML front matter가 닫히지 않았습니다.") from error

    front_matter_text = "\n".join(lines[1:end_index])
    try:
        metadata = yaml.safe_load(front_matter_text) or {}
    except yaml.YAMLError as error:
        raise PostParseError(f"{path}: YAML front matter를 읽을 수 없습니다.") from error

    if not isinstance(metadata, dict):
        raise PostParseError(f"{path}: YAML front matter는 mapping이어야 합니다.")
...
def _require_string(metadata: dict[str, Any], field: str, path: Path) -> str:
    ...
    if not isinstance(value, str) or not value.strip():
        raise PostParseError(f"{path}: '{field}'는 비어 있지 않은 문자열이어야 합니다.")
    return value
...
def _build_post(metadata: dict[str, Any], body_path: Path, body: str) -> Post:
    ...
    if missing:
        fields = ", ".join(missing)
        raise PostParseError(f"{body_path}: 필수 메타데이터가 없습니다: {fields}")
    ...
    if not isinstance(tags, list) or not all(isinstance(tag, str) and tag for tag in tags):
        raise PostParseError(f"{body_path}: 'tags'는 비어 있지 않은 문자열 목록이어야 합니다.")
